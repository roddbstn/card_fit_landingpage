const AnalysisEngine = {
    ...
    analyze(transactions, cardId = 'samsung_taptap') {
        ...
        // Calculate potential benefits and gaps (simplified for MVP)
        let totalMaxPossible = 0;
        Object.entries(card.rules).forEach(([key, rule]) => {
            const max = rule.monthlyLimit || (rule.fixedBenefit * rule.monthlyCountLimit) || 0;
            totalMaxPossible += max;
        });

        result.potentialBenefits = totalMaxPossible;
        result.missedBenefits = Math.max(0, totalMaxPossible - result.currentBenefits);

        // Always recommend NH Flex for User, Samsung for Guest initially
